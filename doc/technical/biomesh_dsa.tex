...
\end{itemize}
\\
The \texttt{Seed Plane Generation} routine provides the surface on which the \texttt{Seed Points} are computed. This surface contains the Seed Points, these points mark the beginning of a fiber.\\
\\
...
\begin{algorithmic}[1]
    ...
    \State $t \gets 0$
    \State $t_{max} \gets time\_step\_max(T)$
...
\right)$
     ...
     \While{$(C(cid) = 1 \lor C(cid) = 2) \And t < t_{max}$}
