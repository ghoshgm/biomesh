...
\item \textbf{Fiber Generation}
    ...
    \texttt{\textbf{Algorithm:}}
\begin{algorithmic}[1]
    \Require Structured grid $G$, cell classification map $C$, configuration parameters $T$
    ...
    \State $N_{\text{adaptive}}^{\max} \gets \text{adaptive\_steps\_max}(T)$
     ...
     \While{$t_{\text{start}} < t_{\text{end}} \;\land\; s \in G$}
        ...
        \If{$C(i) = 0$}
            \If{$\text{adaptivity}$ }
                \If{$N_{\text{adaptive}}^{\max}  \text{reached}$}
                    \State \textbf{break}
                \Else
