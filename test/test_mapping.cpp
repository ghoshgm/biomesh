...
int main(int argc, char** argv)
{
  ...
  /* Test for 3D inverse isoparametric mapping for unit cube. */
  {
    PetscErrorCode ierr;
    ...
    vertex3D coords(0.5, 0.5, 1.0);
    vertex3D ref_result(0.5, 0.5, 1.0);
    vertex3D result = mapping::inverse_isoparametric<vertex3D>(coords, vertices);
    std::cout << "------------------" << std::endl;
    ref_result.print();
    result.print();
    std::cout << "------------------" << std::endl;
    assert(std::fabs(result('x') - ref_result('x')) < 1e-8);
    assert(std::fabs(result('y') - ref_result('y')) < 1e-8);
    assert(std::fabs(result('z') - ref_result('z')) < 1e-8);
